...
import { Unidade_medida } from 'src/app/modulo-estoque/entities/unidade-medida';
import { Unidade_medidaService } from 'src/app/modulo-estoque/services/unidade-medida-service';
...
export class UnidadeMedidaCrudComponent implements OnInit {
  ...
  constructor(private estadoService: Unidade_medidaService) { }

  ngOnInit() {
    this.estadoService.list().subscribe(unidade_medidas => this.unidade_medidas = unidade_medidas);
    this.cols = [
  ...
  delete() {
    ...
    if(confirm("Deseja realmente excluir esse registro?")){
      this.estadoService.delete(this.selectedUnidade_medida.id).subscribe(response => {
            let res:Response = <Response>response;
            if(res.codigo == 1){
              alert(res.mensagem);
